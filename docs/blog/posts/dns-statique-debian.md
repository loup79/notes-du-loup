...
<figure markdown>
  ![Synoptique - DNS statique sous Debian 12 - Mémento 7.1](../images/2026/02/dns-statique-deb12-memento-7.1.webp){ width="430" }
</figure>

## Mémento 7.1 - DNS avec BIND 9

Le service DNS statique reposera sur [BIND 9](https://www.isc.org/bind/){ target = "_blank" } qui sera installé sur srvlan pour gérer la zone LAN.
...
```bash
[user@hostname] ip address ou ip a
```
...
b) Une Box Internet déclarée serveur DNS pour la zone LAN.
...
Ex : ping debian-vm1 depuis la VM debian-vm1
...
<figure markdown>
  ![Synoptique - DNS : Fonctionnement de la récursion DNS](../images/2026/02/dns-recursif.webp){ width="430" }
...
```markdown
BIND 9.20.18-1~deb13u1-Debian (Stable Release) <id:>
```
...
tcp   LISTEN 0      10      192.168.3.1:53      0.0.0.0:*    users:(("named"...))
```
...
La ligne de Cde rndc permet de gérer le démon named localement ou à distance.
...
| 86400 {.td-bord}   | Si échec, il réessaiera 1d après  {.td-bord}                                  |
...
!!! note "Nota"
    Le conteneur sécurisé rootless ctn3 n'a pas été traité, le service réseau slirp4netns ou pasta qui lui est associé manque de certaines fonctionnalités notamment celle de ne pas donner au conteneur une adresse IP routable.
...
;; AUTHORITY SECTION:
intra.loupipfire.fr. 86400 IN SOA srvlan.intra.loupipfire.fr. root.intra.loupipfire.fr. 1 604800 86400 2419200 604800
...
```bash
[srvlan@srvlan] sudo named-checkconf -z 
```
...
Actuellement bind9 traite les requêtes locales _(Ex : ping debian-vm1.intra.loupipfire.fr)_ ainsi que les requêtes d'accès à Internet _(Ex : ping lemonde.fr)_ et construit son cache en fonction.
...
```bash
[srvlan@srvlan] nslookup lemonde.fr
```
...
Se connecter en SSH sur la VM ovs _([voir Mémento 6.1](../posts/controle-distant-debian.md){ target="_blank" })_.
...
!!! note "Nota"
    Les 4 premières lignes sont présentes seulement sous Debian 13.
...
et lancez la Cde cat /etc/resolv.conf qui doit retourner ceci :
...
Le fichier resolv.conf a été mis à jour automatiquement lors de la reconstruction des 2 conteneurs Podman.
...
```bash
[clientlinux@debian-vm*]  sudo apt install dnsutils
```

!!! note "nota"
    Non nécessaire sous Debian 13.

et vérifiez l'IP du Name Server pour intra.loupipfire.fr :

```bash
[clientlinux@debian-vm*] nslookup ovs.intra.loupipfire.fr
```
...
[clientlinux@debian-vm*] ping ovs
```
...
Redémarrez les clients debian-vm* et ovs afin de vider leurs caches DNS.

Les pings du [§ VM debian-vm*](#vm-debian) lancés depuis les VM debian-vm* devraient échouer.
...
[Mémento 7.2](../posts/dhcp-kea-debian.md){ .md-button .md-button--primary }
