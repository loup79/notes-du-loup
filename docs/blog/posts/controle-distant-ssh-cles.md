...
L'[authentification par clés](https://www.it-connect.fr/chapitres/authentification-ssh-par-cles/){ target = "_blank" }, méthode plus sécurisée que celle faisant appel au MDP, repose sur une paire de clés cryptographiques ==publique== et ==privée== créée sur le client.
...
!!! Nota "Nota"
    La passphrase est facultative mais celle-ci augmente la sécurité et est donc conseillée.
