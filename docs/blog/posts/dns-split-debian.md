...
<figure markdown>
  ![Synoptique - DNS split sous Debian - Mémento 10.1](../images/2026/02/dns-split-delegation-de-zone-reverse-proxy-debian-memento-10.1.webp){ width="430" }
</figure>

## Mémento 10.1 - DNS Split-Brain

Rappel, 2 entités distinctes coexistent sur le réseau virtuel :  
\- Le domaine loupvirtuel.fr en zone interne publique.  
\- Le sous-domaine intra.loupipfire.fr en zone interne privée.
...
\- 1 sur la VM srvdmz pour le domaine loupvirtuel.fr _(création)_.  
\- 1 sur la VM srvlan pour le sous-domaine intra.loupvirtuel.fr.
...
Fournir une IP publique aux [requêtes DNS](https://blog.stephane-robert.info/docs/reseaux/fondamentaux/dns/){ target="_blank" } provenant de la zone Internet et une IP locale aux requêtes provenant des zones internes DMZ/LAN.

Cela évitera notamment de dévoiler au monde extérieur l'IP interne du domaine loupvirtuel.fr.

<figure markdown>
  ![Synoptique - DNS split : Principe](../images/2026/04/schema-dns-split-memento-10.1.webp){ width="430" }
  <figcaption>DNS split : Principe</figcaption>
</figure>
...
<figure markdown>
  ![Synoptique - Reverse Proxy : Principe](../images/2026/04/reverse-proxy-memento-10.1.webp){ width="430" }
  <figcaption>Reverse Proxy : Principe</figcaption>
</figure>
...
Installez ensuite le même serveur DNS que sur la VM srvlan :
...
\- Les fichiers de zone seront créés dans /var/lib/bind/.

#### _- Sécurisation avec clé TSIG_

La **T**ransaction **Sig**nature sécurisera la liaison entre les serveurs DNS des VM srvdmz et srvlan.
...
[root@srvdmz] sudo tsig-keygen -a hmac-sha256 tsig-dns-split > /etc/bind/tsig-dns-split.key

[root@srvdmz] chown root:bind tsig-dns-split.key
...
Affichez le contenu de la clé tsig-dns-split :
...
key "tsig-dns-split" {
        ...
        secret "8ZcCWWHIG8shAijz1zs9WJJzUwu...";
};
```

Editez, pour déclarer l'usage de la clé, le fichier named.conf.local :
...
```markdown
# Déclaration clé TSIG
include "/etc/bind/tsig-dns-split.key";
...
et commentez selon votre version Debian la ligne suivante :

```markdown
#include "/etc/bind/named.conf.default-zones";   # Sous Debian 12
#include "/etc/bind/named.conf.root-hints";      # Sous Debian 13
```
...
et créez les ACL, les vues, les zones de recherche et délégations en ajoutant ceci à la fin du fichier :

-- Pour **Debian 12**, entrez ce contenu --

```markdown hl_lines="6 44-45 50"
...
recursion yes;
allow-recursion { 192.168.2.0/24; 192.168.4.0/24; };
...
allow-update { none; };
allow-transfer { 192.168.3.1; key tsig-dns-split; };
};
...
-- Pour **Debian 13** entrez ce contenu --

```markdown hl_lines="6 12-16 53 58-62"
## Définition des ACL pour les zones internes/externes
acl interne { 192.168.2.0/24; 192.168.4.0/24; };
acl externe { any; };
...
Sous Debian 13, les zones par défaut ne sont plus déclarées globalement mais à l'intérieur de chaque vue.

Contrôlez ensuite la syntaxe du fichier et redémarrez Bind 9 :
...
intra           IN NS   srvlan.intra.loupvirtuel.fr.
srvlan.intra    IN A    192.168.3.1
```

2026022001 = créé le 20/02/2026 + index de départ 01

Le fichier contient les enregistrements DNS permettant d'accéder au domaine loupvirtuel.fr ainsi qu'une déclaration de délégation de zone pour l'accès au sous-domaine intra.loupvirtuel.fr.
...
Avant, contrôlez la syntaxe des fichiers de zone ci-dessus et redémarrez Bind 9 :
...
Le statut signale pour l'instant un réseau injoignable _(network unreachable resolving)_.
...
et vérifiez qu'il contient bien le FQDN de la VM srvdmz :
...
Videz ensuite le contenu du fichier DNS /etc/resolv.conf puis désactivez son remplissage automatique par le service de NetworkManager comme suit :
...
Comme dans le [mémento 7.1](../posts/dns-statique-debian.md#dns-externe){target="_blank"}, vous allez faire en sorte que les demandes de résolution externe (Ex : ping `www.yahoo.fr`) soient traitées par le serveur DNS de la box Internet.
...
Le statut montre cette fois les zones DNS chargées _(plus de network unreachable resolving)_ .
...
et ajoutez ce qui suit au dessus de # Pas d'utilisation ... :
...
```markdown hl_lines="3 17-19"
● named.service - BIND Domain Name Server
     Loaded: loaded (/usr/lib/systemd/system/named.service; enabled; preset: enabled)
     Active: active (running) since Mon 2026-02-23 11:10:43 CET; 12s ago
 Invocation: 00db20510672480883fb3a694bedf7ce
       Docs: man:named(8)
   Main PID: 2471 (named)
     Status: "running"
      Tasks: 6 (limit: 1675)
     ...
     CGroup: /system.slice/named.service
             └─2471 /usr/sbin/named -f -u bind

févr. 23 11:10:43 srvdmz named[2471]: command channel listening on 127.0.0.1#953
févr. 23 11:10:43 srvdmz named[2471]: configuring command channel from '/etc/bind/rndc.key'
févr. 23 11:10:43 srvdmz named[2471]: command channel listening on ::1#953
févr. 23 11:10:43 srvdmz named[2471]: zone 4.168.192.in-addr.arpa/IN/interne: loaded serial 2026022001
févr. 23 11:10:43 srvdmz named[2471]: zone loupvirtuel.fr/IN/interne: loaded serial 2026022001
févr. 23 11:10:43 srvdmz named[2471]: zone loupvirtuel.fr/IN/externe: loaded serial 2026022001
févr. 23 11:10:43 srvdmz named[2471]: all zones loaded
févr. 23 11:10:43 srvdmz named[2471]: FIPS mode is disabled
févr. 23 11:10:43 srvdmz systemd[1]: Started named.service - BIND Domain Name Server.
févr. 23 11:10:43 srvdmz named[2471]: running
```
...
Accédez à l'interface Web d'IPFire depuis la VM srvlan, puis :  
Menu IPFire -> Pakfire  
...
-> Nom : ==srvdmz== -> IP/MAC : Entrez 192.168.4.2  
-> Remarque : Entrez Serveur en zone orange  
-> Bouton Sauvegarder

-> Nom : ==srvlan== -> IP/MAC : Entrez 192.168.3.1  
-> Remarque : Entrez Serveur en zone verte  
-> Bouton Sauvegarder -> Bouton Retour

Menu Pare-feu -> Règles de pare-feu  
-> Bouton Nouvelle règle

a ) Ouverture du port HTTPS 443, Internet vers la VM srvdmz :  
Nécessaire pour accéder à loupvirtuel.fr depuis Internet.

Paramétrage :  
\- Source -> Cochez Réseaux standards -> Sélectionnez ROUGE  
\- NAT -> Cochez Utiliser la traduction d'adresses réseau (NAT)  
\- Destination -> Cochez Adresse IP de ... -> Entrez 192.168.4.2  
\- Protocole -> Sélectionnez TCP -> Port de dest... -> Entrez 443  
...
<figure markdown>
  ![Capture - IPFire : HTTP/S ouverts zone Rouge vers Orange](../images/2026/02/debian-ipfire-dns-split-1.webp){ width="430" }
  <figcaption>IPFire : HTTP/S ouverts zone Rouge vers Orange</figcaption>
</figure>

b ) Ouverture du port DNS 53, VM srvdmz vers VM srvlan :  
...
Paramétrage :  
\- Source -> Cochez Hôtes -> Sélectionnez srvdmz  
\- NAT -> Cochez Utiliser la traduction d'adresses réseau (NAT)  
\- Destination -> Cochez Hôtes -> Sélectionnez srvlan  
\- Protocole -> Sélectionnez TCP -> Port de dest... -> Entrez 53  
\- Paramètres additionnels -> Remarque  
...
<figure markdown>
  ![Capture - IPFire : DNS ouvert zone Orange vers Verte](../images/2026/02/debian-ipfire-dns-split-2.webp){ width="430" }
  <figcaption>IPFire : DNS ouvert zone Orange vers Verte</figcaption>
</figure>

c ) Ouverture du port DNS 53, Internet vers la VM srvdmz :  
...
\- NAT -> Cochez Utiliser la traduction d'adresses réseau (NAT)  
\- Destination -> Cochez Adresse IP de … -> Entrez 192.168.4.2  
\- Protocole -> Sélectionnez UDP -> Port de dest... -> Entrez 53  
\- Paramètres additionnels -> Remarque  
-> Entrez DNS port UDP Internet vers srvdmz autorisé  
-> Bouton Ajouter -> Bouton Appliquer les changements

<figure markdown>
  ![Capture - IPFire : DNS UDP ouvert zone Rouge vers Orange](../images/2026/02/debian-ipfire-dns-split-3.webp){ width="430" }
  <figcaption>IPFire : DNS UDP ouvert zone Rouge vers Orange</figcaption>
</figure>

d ) Ouverture du port HTTP 80, VM srvdmz vers VM srvlan :  
Règle **temporaire** créée pour **tester** l'accès au serveur Web de la VM srvlan depuis Internet ou la VM srvdmz mais représentant un trou de sécurité entre la zone publique DMZ et la zone privée LAN.

Paramétrage :  
\- Source -> Cochez Hôtes -> Sélectionnez srvdmz  
\- NAT -> Cochez Utiliser la traduction d'adresses réseau (NAT)  
\- Destination -> Cochez Hôtes -> Sélectionnez srvlan  
\- Protocole -> Sélectionnez TCP -> Port de dest... -> Entrez 80  
\- Paramètres additionnels -> Remarque  
-> Entrez HTTP vers serveur Web zone verte autorisé  
-> Bouton Ajouter -> Bouton Appliquer les changements

<figure markdown>
  ![Capture - IPFire : HTTP ouvert zone Orange vers Verte](../images/2026/02/debian-ipfire-dns-split-4.webp){ width="430" }
...
```markdown hl_lines="2 4 6 8 14 16 18 20"
...
allow-transfer { 192.168.4.2; key tsig-dns-split; };
};
...
Supprimez la ligne 192.168.4.2 loupvirtuel.fr, c'est le serveur DNS de la VM srvdmz qui fournira dorénavant cette correspondance IP/Nom de domaine.
...
192.168.4.2 = IP du serveur DNS de la VM srvdmz
...
Au préalable, redémarrez la VM srvdmz sinon la VM srvlan ne sera pas en mesure de joindre le monde extérieur pour notamment installer un nouveau paquet Linux. En effet c'est maintenant la VM srvdmz qui gère le résolution DNS externe.

Puis, installez le paquet Apache :
...
Erreur observée : AH00558: apache2: Could not reliably determine the server's fully qualified domain name

Entrez ces Cdes pour régler ce problème du FQDN :
...
Ensuite, depuis la VM srvdmz, testez l'URL `http://192.168.3.1` qui doit retourner la page d'accueil par défaut d'Apache.
...
Pour finir, testez de nouveau l'URL `http://192.168.3.1` qui doit cette fois montrer le nouveau contenu.

### Modification sur VM zone LAN

\- VM ovs

Redémarrez la VM ovs et vérifiez la MAJ automatique de son fichier DNS /etc/resolv.conf ainsi que ceux des conteneurs Podman ctn1/2 et ctn3 _(Réf. [mémento 5.2](../posts/podman-debian-lxc-partie-1.md){target="_blank"})_.
...
```bash
[clientlinux@debian-vm*] sudo systemctl restart NetworkManager

[clientlinux@debian-vm*] cat /etc/resolv.conf
```

Editez le fichier DNS hosts :

```bash
[clientlinux@debian-vm*] sudo nano /etc/hosts
```
...
Par curiosité, observez au bout de quelques minutes la bonne MAJ dynamique du fichier de zone DNS /var/lib/bind/db.intra.loupvirtuel.fr.directe situé sur la VM srvlan :

```markdown hl_lines="2"
$TTL 86400  ; 1 day
intra.loupvirtuel.fr.           IN SOA    srvlan.intra.loupvirtuel.fr. root.intra.loupvirtuel.fr. (
                                          5          ; serial
                                ...
                                NS  srvlan.intra.loupvirtuel.fr.
                                 A  192.168.3.1
$TTL 9600  ; 2 hours 40 minutes
ctn1.intra.loupvirtuel.fr.       A  192.168.3.20
                             DHCID  ( AAIBgMQ/vZs4VhzP2fOQ4rNyJb5a8znkTcoHHManuC38
                                    dHE= ) ; 2 1 32
ctn2.intra.loupvirtuel.fr.       A  192.168.3.30
                             DHCID  ( AAIBH5DkixBUDAOD3bh0DrY/Sav62e4+IdAQqeFaCLZm
                                    rHw= ) ; 2 1 32
debian-vm1.intra.loupvirtuel.fr. A  192.168.3.31
                             DHCID  ( AAEBFmf++gXvelzEH0qCmVejU0yQY1cVjPwsJj9SVFTm
                                    LHk= ) ; 1 1 32
debian-vm2.intra.loupvirtuel.fr. A  192.168.3.32
                             DHCID  ( AAEBpxD8v96lC1rmddrsmr9fl/Kz59CVncESXTouT0wM
                                    3OY= ) ; 1 1 32
$TTL 86400  ; 1 day
ovs.intra.loupvirtuel.fr.        A  192.168.3.15
srvlan.intra.loupvirtuel.fr.     A  192.168.3.1
www.intra.loupvirtuel.fr.    CNAME  srvlan.intra.loupvirtuel.fr.
```
...
a ) Vérifiez l'usage de TSIG entre les VM srvdmz et srvlan :

```bash
[srvdmz@srvdmz] sudo dig -y hmac-sha256:tsig-dns-split:8ZcCWWHIG8shAijz1zs9WJJzUwu... @192.168.3.1 AXFR intra.loupvirtuel.fr
```

Retour normal:

```markdown hl_lines="19"
; <<>> DiG ... <<>> -y hmac-sha256:tsig-dns-split:8ZcCW... @192.168.3.1 AXFR intra.loupvirtuel.fr
; (1 server found)
;; global options: +cmd
intra.loupvirtuel.fr. 86400 IN  SOA  srvlan.intra.loupvirtuel.fr. root.intra.loupvirtuel...
intra.loupvirtuel.fr. 86400 IN  NS  srvlan.intra.loupvirtuel.fr.
intra.loupvirtuel.fr. 86400 IN  A  192.168.3.1
ctn1.intra.loupvirtuel.fr. 9600 IN  A 192.168.3.20
ctn1.intra.loupvirtuel.fr. 9600 IN  DHCID AAIBgMQ/vZs4VhzP2fOQ4rNyJb5a8znkTcoHHManuC38dHE=
ctn2.intra.loupvirtuel.fr. 9600 IN  A 192.168.3.30
ctn2.intra.loupvirtuel.fr. 9600 IN  DHCID AAIBH5DkixBUDAOD3bh0DrY/Sav62e4+IdAQqeFaCLZmrHw=
debian-vm1.intra.loupvirtuel.fr. 9600 IN A 192.168.3.31
debian-vm1.intra.loupvirtuel.fr. 9600 IN DHCID AAEBFmf++gXvelzEH0qCmVejU0yQY1cVjPwsJj9SVFTmLHk=
debian-vm2.intra.loupvirtuel.fr. 9600 IN A 192.168.3.32
debian-vm2.intra.loupvirtuel.fr. 9600 IN DHCID AAEBpxD8v96lC1rmddrsmr9fl/Kz59CVncESXTouT0wM3OY=
ovs.intra.loupvirtuel.fr. 86400 IN  A 192.168.3.15
srvlan.intra.loupvirtuel.fr. 86400 IN A 192.168.3.1
www.intra.loupvirtuel.fr. 86400 IN  CNAME srvlan.intra.loupvirtuel.fr.
intra.loupvirtuel.fr. 86400 IN  SOA srvlan.intra.loupvirtuel.fr. root.intra.loupvirtuel...
tsig-dns-split.         0 ANY TSIG  hmac-sha256. 1771865027 300 32 2WwgfO... 35721 NOERROR 0 
;; Query time: 8 msec
;; SERVER: 192.168.3.1#53(192.168.3.1) (TCP)
;; WHEN: Mon Feb 23 17:43:49 CET 2026
;; XFR size: 15 records (messages 1, bytes 620)
```
...
```markdown hl_lines="3"
loading "loupvirtuel.fr" from "db.loupvirtuel.fr.directe" class "IN"
zone loupvirtuel.fr/IN: loaded serial 2026022001
OK
```
...
```markdown hl_lines="3"
loading "3.168.192.in-addr.arpa" from "db.loupvirtuel.fr.inverse" class "IN"
zone 3.168.192.in-addr.arpa/IN: loaded serial 2026022001
OK
```
...
```markdown hl_lines="2 6 16 20 27 31 38 42"
...
Received 69 bytes from 192.168.4.2#53 in 0 ms
Trying "srvdmz.loupvirtuel.fr"
...
;; AUTHORITY SECTION:
loupvirtuel.fr. 86400 IN SOA srvdmz.loupvirtuel.fr. root.loupvirtuel.fr. 2026022001 604800 86400 2419200 604800
...
;; QUESTION SECTION:
;srvdmz.loupvirtuel.fr. IN HTTPS
...
```markdown hl_lines="4 11"
; <<>> DiG 9.20.18-1~deb13u1-Debian <<>> SOA srvdmz.loupvirtuel.fr
;; global options: +cmd
;; Got answer:
...
; COOKIE: 4022e0119f8da95901000000699d7e1e46b39c573655a65e (good)
;; QUESTION SECTION:
...
;; WHEN: Tue Feb 24 11:31:58 CET 2026
;; MSG SIZE  rcvd: 119
```
...
a ) Vérifiez l'usage de TSIG entre les VM srvlan et srvdmz :

```bash
[srvlan@srvlan] sudo dig -y hmac-sha256:tsig-dns-split:8ZcCWWHIG8shAijz1zs9WJJzUwu... @192.168.4.2 AXFR loupvirtuel.fr
```

et contrôlez les résutats des test suivants en comparant avec ceux pratiqués sur la VM srvdmz :
...
Redémarrez les VM debian-vm* et vérifiez sur la VM srvlan que le fichier des baux /var/lib/kea/kea-leases4.csv.2 montre à présent le sous-domaine intra.loupvirtuel.fr.

c ) Contrôlez le résultat des pings suivants :

```bash
[srvlan@srvlan] ping debian-vm1.intra
[srvlan@srvlan] ping debian-vm2.intra.loupvirtuel.fr

[srvlan@srvlan] ping srvlan # 127.0.1.1
[srvlan@srvlan] ping srvlan.intra.loupvirtuel.fr # 192.168.3.1

[srvlan@srvlan] ping ovs.intra
[srvlan@srvlan] ping ctn1.intra.loupvirtuel.fr
[srvlan@srvlan] ping ctn2.intra
...
#### _- Côté clients debian-vm*_

Testez les 2 Cdes suivantes :

```bash
[clientlinux@debian-vm*] dig SOA www.loupvirtuel.fr
[clientlinux@debian-vm*] dig SOA www.yahoo.fr 
```
...
```bash
[clientlinux@debian-vm*] nslookup www.loupvirtuel.fr
[clientlinux@debian-vm*] nslookup 192.168.4.2  
```
...
<figure markdown>
  ![Capture - Windows : IPv6 à désactiver pour la simulation](../images/2026/02/win11-dns-split-config-ip-serveur-dns_1.webp){ width="430" }
...
<figure markdown>
  ![Capture - Windows : IPv4 - DNS préféré = 192.168.x.w](../images/2026/02/win11-dns-split-config-ip-serveur-dns_2.webp){ width="430" }
...
Toutes les deux doivent basculer vers le protocole https sous réserve d'avoir au préalable importé le certificat ssl loupvirtuel-ca.pem dans le magasin des autorités gérées par le navigateur Web (voir [mémento 8.1](../posts/lamp-https-cms-partie-2-debian.md#pem-firefox){target="_blank"}).
...
Le retour affiché doit être l'accueil du site Web de la VM srvlan.
...
[Mémento 11.1](../posts/e-mail-partie-1-debian.md){ .md-button .md-button--primary }
