...
<figure markdown>
  ![Synoptique - FTP avec VsFTPd sous Debian - Mémento 9.2](../images/2026/02/ftp-ftps-serveur-vsftpd-debian-memento-9.2.webp){ width="430" }
</figure>
...
Le [FTP](https://cpisoftware.eu/blog-post-ftp){ target="_blank" } est un protocole de communication dédié au transfert de fichiers sur un réseau TCP/IP.

Les utilisateurs des VM debian-vm* _(clients)_ utiliseront la VM srvlan _(serveur)_ pour déposer ou s'échanger des fichiers.
...
Vérifiez ensuite le bon démarrage de celui-ci :
...
```markdown hl_lines="3 14"
● vsftpd.service - vsftpd FTP server
     Loaded: loaded (/usr/lib/systemd/system/vsftpd.service; enabled; preset: enabled)
     Active: active (running) since Thu 2026-02-19 10:50:45 CET; 20s ago
 Invocation: 0d14a2011a6b45a886e9ed892306a6c2
    Process: 3169 ExecStartPre=/bin/mkdir -p /var/run/vsftpd/empty (code=exited, status=0/SUCCESS)
   Main PID: 3170 (vsftpd)
      Tasks: 1 (limit: 1675)
     Memory: 896K (peak: 1.7M)
        CPU: 35ms
     CGroup: /system.slice/vsftpd.service
             └─3170 /usr/sbin/vsftpd /etc/vsftpd.conf

févr. 19 10:50:45 srvlan systemd[1]: Starting vsftpd.service - vsftpd FTP server...
févr. 19 10:50:45 srvlan systemd[1]: Started vsftpd.service - vsftpd FTP server.
```
...
Pour les tests à venir, copiez ces 2 fichiers dans ce dossier :
...
Email Address []: Appuyez sur la touche Entrée
```
...
Le [synoptique du mémento](../images/2026/02/ftp-ftps-serveur-vsftpd-debian-memento-9.2.webp){:target="_blank"} détaille l'intérêt de ce mode.
...
Installez le client FTP de nom lftp sur debian-vm2 :

```bash
[clientlinux@debian-vm2] sudo apt install lftp
```
...
```bash
[clientlinux@debian-vm2] cd /home/clientlinux
[clientlinux@debian-vm2] nano .lftprc
```
...
Enfin, connectez-vous sur le serveur vsftpd de la VM srvlan et entrez la Cde ls :

```bash
[clientlinux@debian-vm2] lftp srvlan
...
```bash
[clientlinux@debian-vm2] cd Documents
[clientlinux@debian-vm2] lftp srvlan
...
```bash
[clientlinux@debian-vm2] touch test-ftp-vm2
[clientlinux@debian-vm2] echo "Test Upload FTP" > test-ftp-vm2
```

et testez un envoi FTP dans le dossier public de la VM srvlan :
...
```markdown hl_lines="3"
put: L'accès a échoué : 550 Permission denied. (test-ftp-vm2)
...
Installez sur la VM debian-vm2 le client FTP/SFTP FileZilla :

```bash
[clientlinux@debian-vm2] sudo apt install filezilla
```
...
<figure markdown>
  ![Capture - FileZilla : Réglages généraux FTP Anonymous](../images/2026/02/ftps-debian-anonymous-1.webp){ width="430" }
...
<figure markdown>
  ![Capture - FileZilla : Réglages avancés FTP Anonymous](../images/2026/02/ftps-debian-anonymous-2.webp){ width="430" }
...
<figure markdown>
  ![Capture - FileZilla : Connexion FTP Anonymous établie](../images/2026/02/ftps-debian-anonymous-3.webp){ width="430" }
...
Accédez au dossier suivant de la VM debian-vm2 :
...
Pour cela, vous allez créer un utilisateur local de nom userftps sans MDP associé, ce qui interdira toute tentative de connexion avec ce nom sur la VM srvlan.
...
Un MDP UNIX sera demandé pour chacun d'eux. Vous pouvez utiliser ceux des utilisateurs des VM debian-vm* mais ce n'est pas obligatoire. Vous êtes également libre de répondre ou non aux demandes d'information qui apparaîtront.
...
Créez sur le client FileZilla de debian-vm2 un site de nom srvlan-commun.

Paramétrez celui-ci comme montré ci-dessous :  
\- Onglet Général

<figure markdown>
  ![Capture - FileZilla : Création du site FTPS srvlan-commun](../images/2026/02/ftps-debian-dossier-commun-1.webp){ width="430" }
  <figcaption>FileZilla : Création du site FTPS srvlan-commun</figcaption>
</figure>

\- Onglet Avancé  
-> Dossier local par défaut -> Parcourir...  
-> Sélectionnez /home/clientlinux/Documents  
-> Bouton Ouvrir -> Bouton Valider
...
<figure markdown>
  ![Capture - FileZilla : Certificat SSL émis par le serveur vsftpd](../images/2026/02/ftps-debian-dossier-commun-2.webp){ width="430" }
...
<figure markdown>
  ![Capture - FileZilla : Connexion sécurisée FTPS établie](../images/2026/02/ftps-debian-dossier-commun-3.webp){ width="430" }
...
Pour finir, créez sur le client FileZilla de debian-vm1 le site srvlan-commun avec comme utilisateur local clientvm1 et contrôlez après connexion que vous retrouvez bien les mêmes permissions que ci-dessus.

#### _- Sur tous les dossiers_

Choix **déconseillé**, l'utilisateur du FTP pouvant accéder à des fichiers sensibles.

La configuration actuelle enferme les utilisateurs locaux tels clientvm* dans le dossier racine commun_locaux, ceux-ci ne peuvent donc pas accéder au dossier parent donc à l'arborescence complète de la VM srvlan.
...
Créez sur le FileZilla de debian-vm1 le site srvlan-complet et attribuez lui ces paramètres :
...
<figure markdown>
  ![Capture - FileZilla : Accès complet aux dossiers/fichiers de srvlan](../images/2026/02/ftps-debian-acces-complet.webp){ width="430" }
...
!!! note "Nota"
    Les utilisateurs ci-dessus, inconnus de srvlan, peuvent avoir des MDP différents ou non de ceux affectés aux utilisateurs locaux des VM debian-vm*.
...
Mappage sur l'utilisateur userftps et attribution des mêmes privilèges que ceux des utilisateurs locaux de la VM srvlan.
...
Créez depuis FileZilla les sites srvlan-clientvm*-virtuel sur les VM debian-vm* comme suit :
...
Pour les MDP, voir le [§ Création Bdd Berkeley](#titre-berkeley).
...
\- Peut Supprimer/Modifier ses dépôts sur la VM srvlan.  
...
[Mémento 10.1](../posts/dns-split-debian.md){ .md-button .md-button--primary }
